...
const task = async () => {
        ...
        ...listOfUsers.Users.find(
          (cogUser) =>
            (cogUser.Attributes?.length > 0
              ? cogUser.Attributes.find((attr) => (attr.Name = "sub")).Value ??
                ""
  ...
  const mappedAccount = mergedAccounts.map((acc) => ({
    ...
    primary_user: {
      ...
      family_name: getAttribute(acc.primary_user.Attributes, "family_name"),
      phone_number: getAttribute(acc.primary_user.Attributes, "phone_number"),
      last_login:
