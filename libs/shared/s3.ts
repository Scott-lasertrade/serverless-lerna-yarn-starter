...
import { ImageEntity } from "../entities/utils/ImageEntity";
    ...
    imageList?.length > 0
      ? imageList
          ...
          .filter(({ id: id1, order: order1 }) =>
            existingImages.some(
              ({ id: id2, order: order2 }) => id2 === id1 && order1 !== order2
            )
          )
          .filter(
            (existingImage) =>
...
const Storage = {
  ...
  async get(
    ...
    if (!data) {
      throw new AppError(`Failed to get file ${fileName}, from ${bucket}`);
    }
  ...
  async remove(
    ...
    if (!data) {
      throw new AppError(`Failed to delete file ${fileName}, from ${bucket}`);
    }
    return data;
  },
};
